...
from sebec.terminal import TerminalApp, ItermColors
...
def write_itermcolors(destination: Path, *, light: ItermColors, dark: ItermColors):
    ...
    _append(root_dict, default_colors)
    _append(root_dict, dark_colors, suffix="(Dark)")
    _append(root_dict, light_colors, suffix="(Light)")

    tree = ElementTree.ElementTree(root)
    ElementTree.indent(tree, "\t")

    with open(destination, "wb") as color_file:
        color_file.write(HEADER)
        tree.write(color_file, encoding="utf-8", xml_declaration=False)


def _append(parent, color_map: dict[str, str], *, suffix: str = None):
